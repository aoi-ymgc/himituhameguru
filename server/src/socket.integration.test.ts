...
test("3クライアントで参加・再接続・決着・再戦まで進行できる", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    const incident = await waitForState(host.getState, (state) => state.status === "incident");
    const finderClient = clients.find((client) => client.getState()?.viewerId === incident.firstFinderId)!;
    await emitAck(finderClient.socket, "submitIncident", { title: "テスト用の小さな事件" });
    ...
    for (let step = 0; step < 240; step += 1) {
      ...
      if (actionable?.pending) {
        ...
        const card = currentState?.hand.find((item) => item.type !== "secret" && (item.type !== "deduce" || currentState.firstRoundComplete));
    ...
    const previousFinderId = finished.firstFinderId;
    await emitAck(host.socket, "rematch");
    const rematchIncident = await waitForState(host.getState, (state) => state.status === "incident");
    assert.notEqual(rematchIncident.firstFinderId, previousFinderId);
    const rematchFinder = clients.find((client) => client.getState()?.viewerId === rematchIncident.firstFinderId)!;
    await emitAck(rematchFinder.socket, "submitIncident", { title: "再戦の事件" });
